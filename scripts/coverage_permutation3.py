...
for ct in ['Endo','Micro','Astro','Peri','Oligo']:
    ...
    ct_mean[ct] = np.asarray(ad.X[mask].mean(axis=0)).flatten()
global_mean = np.asarray(ad.X.mean(axis=0)).flatten()

def count_fast(gene_list):
    gidx = np.array([gene_idx[g] for g in gene_list])
    total = 0
    for ct in ['Endo','Micro','Astro','Peri','Oligo']:
        total += int((ct_mean[ct][gidx] > global_mean[gidx]).sum())
    return total

obs_total = count_fast(avail)
print('观察值(三药靶点总富集):', obs_total)

np.random.seed(42)
n_perm = 1000
perm = np.array([count_fast(np.random.choice(var_names, size=len(avail), replace=False)) for _ in range(n_perm)])
