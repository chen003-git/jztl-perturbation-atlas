...
# 人类→小鼠 ortholog（来自 scRNA_mapping.py）
h2m = {
    ...
    'AKR1C3':'Akr1c3','FGFR1':'Fgfr1','SLC6A2':'Slc6a2','CASP9':'Casp9','CASP8':'Casp8','PON1':'Pon1',
...
print('brain shape:', ad.shape)

# 定义细胞类型：每个 leiden cluster 取 5 个 marker score 最大者
ct_scores = ['Endo','Micro','Astro','Peri','Oligo']
ad.obs['ct'] = ad.obs[ct_scores].idxmax(axis=1)
print('\n各细胞类型细胞数:')
print(ad.obs['ct'].value_counts())

# 每个药的靶点映射到小鼠
def herb_mouse_targets(herb_col):
    human = tm[tm[herb_col]==1]['Gene'].tolist()
    mouse = [h2m[h] for h in human if h in h2m]
    return mouse, human

# 计算每个药在每种细胞类型的富集靶点数（靶点平均表达 > 1.5× 全局平均）
X = ad.X
if hasattr(X, 'toarray'):
    X = X.toarray()
X = np.asarray(X)

print('\n=== 复验：各药在脑细胞类型的富集靶点数 (阈值 >1.0x 全局均值) ===')
print('%-12s %-8s %-8s %-8s %-8s %-8s' % ('herb','Endo','Micro','Astro','Peri','Oligo'))
for herb_col, herb_name in [('Gegen','Gegen'), ('Huangqi','Huangqi'), ('Danshen','Danshen')]:
    mouse_targets, _ = herb_mouse_targets(herb_col)
    # 只保留在 h5ad 里的基因
    ...
    for ct in ['Endo','Micro','Astro','Peri','Oligo']:
        ...
        # 对每个靶点，判断 ct_mean > 1.0 * global_mean
        enrich = 0
        for g in avail:
            gi = list(ad.var_names).index(g)
            if ct_mean[gi] > 1.0 * global_mean[gi]:
                enrich += 1
        row.append(str(enrich))
    print('%-12s %-8s %-8s %-8s %-8s %-8s' % tuple(row))
    print('  (%s 可用靶点数: %d/%d)' % (herb_name, len(avail), len(mouse_targets)))
